...
import { CheckCircle, Lock, RefreshCw, ShieldX, Eye, Play, Music } from "lucide-react";
...
export function AssetProofButton({ assetId, assetType, assetUrl }: AssetProofButtonProps) {
  const { viewCount, generateProof, proofStatus, hasProof, isLoading, trackView } = useAssetViews({
    assetId,
    autoTrack: false,
  });
  ...
  const isMediaAsset = assetType?.includes("video") || assetType?.includes("audio");
